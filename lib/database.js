...
exports.DatabaseAPI = function DatabaseAPI(hoodie, options) {
    ...
    if (options.editable_permissions) {
        ...
        var updateDesignDoc = function(hoodie, ddoc_url, ddoc, callback) {
        ...
        // this is mutually exclusive with perUserWriteAccess
        db.revokePublicWriteAccess = function (callback) {
            ...
            updateDesignDoc(hoodie, ddoc_url, ddoc, callback);
        };

        // this is mutually exclusive with publicWriteAccess
        db.grantPerUserWriteAccess = function (callback) {
            var db_url = '/' + encodeURIComponent(options.name);
            var ddoc_url = db_url + '/_design/permissions';
            var role_str = JSON.stringify(write);
            var ddoc = {
                ...
                validate_doc_update: 'function(oldDoc, newDoc, userCtx) {\n' +
                    ...
                    'for (var i = 0; i < userCtx.roles.length; i++) {\n' +
                        ...
                        '}\n' +
                    '}\n' +
                    'regular users can only update their own docs\n' +
                    'if(oldDoc && (oldDoc.createdBy != ("user/" + userCtx.name))) {\n' +
                      'throw({"unauthorized": "Can’t touch this"});\n' +
                    '}\n' +
                  '}\n'
            };
            updateDesignDoc(hoodie, ddoc_url, ddoc, callback);
