...
exports.DatabasesAPI = function (hoodie) {
    ...
    database.add = function (name, callback, _retried) {
        var opt = {data: ''};
        ...
        function (err) {
            if (err && err.error === 'file_exists' && !_retried) {
                // couchdb gives this response in error in some circumstances,
                // retry before reporting back with error
                return database.add(name, callback, true);
            }
            else {
