    ...
    return {
        databases: {
            ...
            head: function (name, callback) {
                var opts = {url: '/' + encodeURIComponent(dbName(name))};
                request.head(createOptions(opts), callback);
            },
            list: function (callback) {
                // TODO: transform this as a stream?
                var opts = {url: '/_all_dbs', json: true};
                request.get(createOptions(opts), function (err, res, body) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    var appdbs = body.filter(function (db) {
                        return (
