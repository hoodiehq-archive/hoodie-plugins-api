...
exports.PluginAPI= function (options) {
    ...
    function DatabaseAPI(name) {
        ...
        function addRoles(users, roles, callback) {
            if (!Array.isArray(users)) {
                // single user name passed
                users = [users];
            }
            async.map(users, function (name, cb) {
                var user = hoodie.user(name);
                user.get(function (err, doc) {
                    if (err) {
                        return cb(err);
                    }
                    var new_roles = _.uniq(doc.roles.concat(roles));
                    user.update({roles: new_roles}, cb);
                });
            },
            callback);
        }

        function removeRoles(users, roles, callback) {
            if (!Array.isArray(users)) {
                // single user name passed
                users = [users];
            }
        ...
        var write_role = 'hoodie:write:' + name;
        var read_role = 'hoodie:read:' + name;

        db.grantWriteAccess = function (users, callback) {
            return addRoles(users, [write_role, read_role], callback);
        };
        db.revokeWriteAccess = function (users, callback) {
            return removeRoles(users, [write_role], callback);
        };
        db.grantReadAccess = function (users, callback) {
            return addRoles(users, [read_role], callback);
        };
        db.revokeReadAccess = function (users, callback) {
            return removeRoles(users, [write_role, read_role], callback);
        };
        db.grantPublicReadAccess = function (callback) {
            return removeSecurity(name, callback);
        };
        db.revokePublicReadAccess = function (callback) {
            return addSecurity(name, callback);
        };
        db.grantPublicWriteAccess = function (callback) {
            db.grantPublicReadAccess(function (err, body, res) {
                ...
                return removePermissionsDDoc(name, callback);
            });
        };
        db.revokePublicWriteAccess = function (callback) {
            return addPermissionsDDoc(name, callback);
        };
    };

    function addSecurity(name, callback) {
    ...
    function removeSecurity(name, callback) {
        var sec_url = '/' + uc(name) + '/_security';
        var sec = {
            admins : {
                names : [],
    ...
    function addPermissionsDDoc(name, callback) {
        var ddoc_url = '/' + uc(name) + '/_design/permissions';
        var role_str = JSON.stringify('hoodie:write:' + name);
        var ddoc = {
    ...
    function removePermissionsDDoc(name, callback) {
        var ddoc_url = '/' + uc(name) + '/_design/permissions';
    ...
    hoodie.database.add = function (name, callback) {
        ...
        hoodie.request('PUT', '/' + uc(name), opt, function (err, body) {
            ...
            addSecurity(name, function (err) {
                if (err) {
                    return callback(err);
                }
                addPermissionsDDoc(name, function (err) {
