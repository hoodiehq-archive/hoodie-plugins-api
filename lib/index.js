...
exports.createAPI = function (options, callback) {
    ...
    plugin_feed.on('change', function (change) {
        if (change.id === 'plugin/' + config.name) {
            var url = hoodie._resolve('/plugins/' + change.id));
            couchr.get(url, function (err, doc) {
                if (!err) {
                    updateLocalPluginConfig(doc);
                }
            });
        }
    });
    ...
    app_feed.on('change', function (change) {
        if (change.id === 'config') {
            couchr.get(hoodie._resolve('/app/config'), function (err, doc) {
                if (!err) {
                    updateLocalAppConfig(doc);
                }
            });
        }
    });
    ...
    function DatabaseAPI(name) {
        ...
        db.findAll = function (type, callback) {
            ...
            if (type) {
                ...
                opt.data.end_key = JSON.stringify(type + '/{}');
            }
